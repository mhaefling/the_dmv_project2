...
RSpec.describe Vehicle do
  ...
  describe '#initialize' do
    it 'can initialize' do
      ...
      expect(@cruz.vin).to be_an(String)
      expect(@cruz.year).to eq(2012)
      expect(@cruz.year).to be_an(Integer)
      expect(@cruz.make).to eq('Chevrolet')
      expect(@cruz.make).to be_an(String)
      expect(@cruz.model).to eq('Cruz')
      expect(@cruz.model).to be_an(String)
      expect(@cruz.engine).to eq(:ice)
      expect(@cruz.engine).to be_an(Symbol)
  ...
  describe '#reg_date' do
    it 'can apply the date the vehicle was registered' do
      expect(@cruz.reg_date).to eq(Date.today)
    end
  end
end
